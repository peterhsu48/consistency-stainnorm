...
class KarrasDenoiser:
    ...
    # https://github.com/Kinyugo/consistency_models/blob/main/consistency_models/consistency_models.py#L107
    def karras_schedule(
        self,
        num_timesteps: int,
    ...
    def consistency_losses(
        ...
        sigmas = self.karras_schedule(num_scales, self.sigma_min, self.sigma_max, self.rho, x_start.device)
        mean = -1.1
        std = 2.0
        ...
        indices = th.multinomial(pdf, x_start.shape[0], replacement=True)
        ...
        x_t_gray = th.cat([x_t, gray], dim=1)

        dropout_state = th.get_rng_state()
        distiller = denoise_fn(x_t_gray, t)
        assert distiller.shape[1:] == (3,96,96), f"{distiller.shape[1:]}"
        ...
        x_t2_gray = th.cat([x_t2, gray], dim=1)
        assert x_t2_gray.shape == x_t_gray.shape, f"{x_t2_gray.shape}, {x_t_gray.shape}"
        distiller_target = target_denoise_fn(x_t2_gray, t2)
        distiller_target = distiller_target.detach()
        assert distiller_target.shape[1:] == (3,96,96), f"{distiller_target.shape[1:]}"
        ...
        if self.loss_norm == "l1":
            ...
            d = math.prod(distiller.shape[1:]) # https://github.com/Kinyugo/consistency_models/blob/main/consistency_models/consistency_models.py
            c = 0.00054 * (d) ** (1/2)
            diffs = ((distiller - distiller_target) ** 2 + c ** 2) ** (1/2) - c
            weights = 1 / (t - t2)
            loss = mean_flat(diffs) * weights
        elif self.loss_norm == "mod_weighting":
            d = math.prod(distiller.shape[1:])
            c = 0.00054 * (d) ** (1/2)
            diffs = ((distiller - distiller_target) ** 2 + c ** 2) ** (1/2) - c
            weights = (indices + 1) ** 2
            loss = mean_flat(diffs) * weights
    ...
    def denoise(self, model, x_t, sigmas, **model_kwargs):
        ...
        assert x_t.shape[1:] == (4,96,96) or x_t.shape[1:] == (5,96,96), f"{x_t.shape}"
        model_output = model(c_in * x_t, rescaled_t, **model_kwargs)
        assert model_output.shape[1:] == (3,96,96), f"{model_output.shape}"
        x_t_wo_gray = x_t[:,:3]
        assert x_t_wo_gray.shape[1:] == (3,96,96)
        denoised = c_out * model_output + c_skip * x_t_wo_gray
...
def karras_sample(
    ...
    ts=None,
    # -- CM-Camelyon -- #
    source_x=None, # input source domain image
    gray=None,
    args=None,
    # -- CM-Camelyon -- #
):
    if args.ilvr:
        logging.info("using ilvr for sampling")
...
def stochastic_iterative_sampler(
    ...
    x = None
    assert source_x is not None, f"must have source_x for SDEdit"
    assert gray is not None, f"must have gray for grayscale conditioning"
    assert args is not None, f"must have args"

    t_max_rho = t_max ** (1 / rho)
    t_min_rho = t_min ** (1 / rho)
    s_in = source_x.new_ones([source_x.shape[0]])

    # -- CM-Camelyon -- #
    if ts[0] != 0:
        # Perform SDEdit image-to-image sampling
        # Get time step where the noisy source domain image (the conditioning image) should start from
        t = (t_max_rho + ts[0] / (steps - 1) * (t_min_rho - t_max_rho)) ** rho
        # Add noise to source domain image
        x = source_x + generator.randn_like(source_x) * np.sqrt(t**2 - t_min**2)
    else:
        t = (t_max_rho + ts[0] / (steps - 1) * (t_min_rho - t_max_rho)) ** rho
        x = generator.randn_like(source_x) * np.sqrt(t**2 - t_min**2)
    # -- CM-Camelyon -- #

    for i in range(len(ts) - 1):
        if args.timestep_offset:
            t = (t_max_rho + (ts[i] - 0.1) / (steps - 1) * (t_min_rho - t_max_rho)) ** rho
        else:
            t = (t_max_rho + ts[i] / (steps - 1) * (t_min_rho - t_max_rho)) ** rho
        xg = th.cat([x, gray], dim=1)
        assert xg.shape[1:] == (4,96,96) or xg.shape[1:] == (5,96,96), f"{xg.shape}"
        x0 = distiller(xg, t * s_in)
        assert x0.shape[1:] == (3,96,96), f"{x0.shape}"
        # ilvr
        if args.ilvr and i != (len(ts) - 1 - 1):
            x0 = (1 - args.ilvr_phi) * x0 + args.ilvr_phi * source_x
